function [] = Run_ODE_neuroDA_with_collocation(SeedNum,DATAFLAG,varargin)
%
...
problemFlags.UseTrueStates.flag=0;
problemFlags.UseTrueParams=0;
problemFlags.HalfConstraints=0;
problemFlags.UseControl=UseControl;
problemFlags.Kinactivation=Kinactivation;
if ~problemFlags.Kinactivation
    est.qKins=0; % we don't have a fixed answer for these
end
%varAnneal=0;
...
Iapp =1e3*dataA(yinds,3); % N x NumVoltageObs
y=dataA(yinds,4);
...
FileName=[ 'Est',num2str(length(paramdetails.pest_ind)),...
    'Seed',num2str(SeedNum),'u',num2str(problemFlags.UseControl),'_',Method_tag,'_ObsFN',dataFileName,'_','NumObs',num2str(NumVoltageObs),DSF_tag,'_',linear_solver,'_','T',ClockString];
...
alpha=alpha0^beta;
start = tic;
paramdetails.NumKineticParams=NumKineticParams;
...
if UseAdaptive % assumes have data at fine sampling rate, but downsample elsewhere
    ...
    numintervals=length(0:dtmodel:tdata_shifted(end))-1;
end
% ---------------------- Set Up the Problem ----------------------------- %
if beta == -1
    alpha=0;
end
[probinfo,upperbound,lowerbound,guess,tau] = setupproblem_HH_struct(y,tdata,...
    Iappdata,dtmodel,paramdetails,Qinv,Rinv,parambounds,pest_ind,numintervals,...
    problemFlags,probinfo);
...
lbx=[-250*ones(compartments,1); zeros(Nstate-compartments,1)];
...
for iv=1:NumVoltageObs
    ...
    if iv>1
        ...
        [probinfonew,upperbound,lowerbound,guessnew,tau] = ...
            setupproblem_HH_struct(y,tdata,Iappdata,dtmodel,paramdetails,...
            Qinv,Rinv,parambounds,pest_ind,numintervals,problemFlags,probinfo);
        %if ~testNoiseless
        %         if ~paramsBounded
    ...
    w0 = [w0; guessnew(probinfonew.xind(1,1:Nstate));];% guess(probinfo.xind(1,2));...
    ...
    if problemFlags.UseControl
        ...
        if ControlAtEnd
          ...
          fcontrol=fobju(eta_u*Uk); % first control penalty, this form to match results
       % fcontrol = alpha_u*fobju(Uk); % better way
        J=J+fcontrol{:};
        if SmoothControl
            ...
            dwu0=[dwu0; 0];
            if PenalizeSlope
                fcontrol=fobju(eta_u*cUk); % this form to match results
    ...
    if strcmp(probinfo.IntegrationScheme,'RK4')
        for k=1:N
            ...
            if problemFlags.UseControl && ObsHere(k)
            ...
            elseif problemFlags.UseControl && ObsHere(k+1)
            ...
            if strcmp(MethodToUse,'Weak')
                ...
                J=J+sum(fobjk_model);
                if ObsHere(k)
                    fmeasure_model=Rinv'.*fobj(Xk{1}-y(obsCounter));
                    J=J+fmeasure_model{:};
                end
            else
                %   if
            ...
            if problemFlags.UseControl && ObsHere(k) % penalize control
    ...
    elseif strcmp(probinfo.IntegrationScheme,'SimpsonHermite')
        ...
        %Ff= Function('Ff', {X0,P,IAPPX1}, {f(X0,P,IAPPX1)}, char('x0','p0','Iapp'),char('xf'));
        %Ffu = Function('Ffu', {X0,P,IAPPX1,YDAT1,U1}, {fu(X0,P,IAPPX1,YDAT1,U1)}, char('x0','p0','Iapp','ydat','u'),char('xf'));
        if problemFlags.UseControl && ObsHere(1) % THIS SHOULD BE ASSUMED TO BE THE CASE ALWAYS.
        ...
        for k=1:N/2 % number of intervals, but not necessarily points.
            ...
            if problemFlags.UseControl && ObsHere(2*k)
            ...
            if problemFlags.UseControl && ObsHere(2*k+1)
            ...
            if ObsHere(2*k)
                fmeasure_model=Rinv'.*fobj(Xkbp1{1}-y(obsCounter+1));
                J=J+fmeasure_model{:};
            end
            if ObsHere(2*k+1)
                fmeasure_model=Rinv'.*fobj(Xkp1{1}-y(obsCounter+1+obsIncreaser));
                J=J+fmeasure_model{:};
            end
            if problemFlags.UseControl && ObsHere(2*k) % penalize control
                  fcontrol=fobju(eta_u*Ukbp1); % uncommented to match results
               % fcontrol = alpha_u*fobju(Ukbp1); % better way
                J=J+fcontrol{:}; % subsequent control penalty
            end
            if problemFlags.UseControl && ObsHere(2*k+1) % penalize control
                 fcontrol=fobju(eta_u*Ukp1);
               % fcontrol=alpha_u*fobju(Ukp1); % better way
                J=J+fcontrol{:}; % subsequent control penalty
            end
            if strcmp(MethodToUse,'Strong')
                if UseSlack
                    ...
                    if ScaleConstraints
                        ...
                        if ~SlackAtEnd
                            w0=[w0; zeros(size(guessnew(probinfonew.xind(k+1,1:Nstate)))); ...
                                zeros(size(guessnew(probinfonew.xind(k+1,1:Nstate))));];
                            lbw=[lbw; -slackeps*ones(size(lbx)); -slackeps*ones(size(lbx))];
                            ubw=[ubw;  slackeps*ones(size(lbx)); slackeps*ones(size(lbx))];
                        end
                    end
            ...
            if strcmp(MethodToUse,'Strong') && SmoothControl
                ...
                if PenalizeSlope
                 ...
                 %   fcontrol = alpha_u*(fobju(dUkp1)); % better way
                    J=J+fcontrol{:};
                end
            end
            if ObsHere(2*k)
...
if RampUpControl
    ...
    FileName=[ 'Est',num2str(length(paramdetails.pest_ind)),...
        'Seed',num2str(SeedNum),'u',num2str(problemFlags.UseControl),ControlNameTag,'_',Method_tag,'_ObsFN',dataFileName,'_','NumObs',num2str(NumVoltageObs),DSF_tag,'_',linear_solver,'_','T',ClockString];
    FileName=strrep(FileName,'.csv','dotcsv');
    FileName=strrep(FileName,'.','pt');
