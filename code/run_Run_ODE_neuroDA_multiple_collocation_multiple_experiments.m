function [] =run_Run_ODE_neuroDA_multiple_collocation_multiple_experiments(SeedNum,DATAFLAG,modeldata,ODEmodel,varargin)
if nargin>4
    ...
    VarLoc = find(strcmpi(varargin, 'ScalingSupplied'));
    if ~isempty(VarLoc)
        ScalingSupplied = varargin{VarLoc(1)+1}; %Get the value after the param name
    else
        ScalingSupplied=struct('provided',false);
    end
    
    VarLoc = find(strcmpi(varargin, 'NonUniform'));
    if ~isempty(VarLoc)
        NonUniform = varargin{VarLoc(1)+1}; %Get the value after the param name
    else
        NonUniform=0;
    end
    
    VarLoc = find(strcmpi(varargin, 'VariableICs'));
    if ~isempty(VarLoc)
        VariableICs = varargin{VarLoc(1)+1}; %Get the value after the param name
    else
        VariableICs=struct('known',false);
        
    end
    
    VarLoc = find(strcmpi(varargin, 'observed_variable_in_experiment'));
    if ~isempty(VarLoc)
        observed_variable_in_experiment = varargin{VarLoc(1)+1}; %Get the value after the param name
    else
        observed_variable_in_experiment=[];
    end
        VarLoc = find(strcmpi(varargin, 'ObsFunLoaded'));
    if ~isempty(VarLoc)
        ObsFunLoaded = varargin{VarLoc(1)+1}; %Get the value after the param name
    else
        ObsFunLoaded=0;
    end
...
load(modeldata,'PDATA','Nstate','compartments')
...
%Kinactivation=1;
%dt_col=.1;
Run_ODE_4dvar_with_collocation_multiple_experiments(SeedNum,DATAFLAG,struct('MethodToUse'...
